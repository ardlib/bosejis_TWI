/*
  ...
  Modified 2020 by Greyson Christoforo (grey@christoforo.net) to implement
  timeouts Modified 2024 by Abhijit Bose (aka. Boseji) to add flexibility of
  length in buffers
*/
...
// loops here if twi_timeout_us == 0 then timeout checking is disabled (the
// previous Wire lib behavior) at some point in the future, the default
// twi_timeout_us value could become 25000 and twi_do_reset_on_timeout could
// become true to conform to the SMBus standard
// http://smbus.org/specs/SMBus_3_1_20180319.pdf
static volatile uint32_t twi_timeout_us = 0ul;
static volatile bool twi_timed_out_flag = false; // a timeout has been seen
static volatile bool twi_do_reset_on_timeout =
...
static volatile size_t twi_masterBufferLength;

static uint8_t twi_txBuffer[TWI_BUFFER_LENGTH];
static volatile size_t twi_txBufferIndex;
static volatile size_t twi_txBufferLength;

static uint8_t twi_rxBuffer[TWI_BUFFER_LENGTH];
static volatile size_t twi_rxBufferIndex;
 ...
 */
uint8_t twi_readFrom(uint8_t address, uint8_t *data, size_t length,
                     uint8_t sendStop) {
  size_t i;
  ...
  if (true == twi_inRepStart) {
    ...
    do {
      ...
      if ((twi_timeout_us > 0ul) &&
          ((micros() - startMicros) > twi_timeout_us)) {
        twi_handleTimeout(twi_do_reset_on_timeout);
        return 0;
      }
    } while (TWCR & _BV(TWWC));
 ...
 * end Output   0 .. success 1 .. length to long for buffer 2 .. address send,
 * NACK received 3 .. data send, NACK received 4 .. other twi error (lost bus
 * arbitration, bus error, ..) 5 .. timeout
 */
uint8_t twi_writeTo(uint8_t address, uint8_t *data, size_t length, uint8_t wait,
                    uint8_t sendStop) {
  size_t i;
 ...
 *          0 ok
 */
uint8_t twi_transmit(const uint8_t *data, size_t length) {
  size_t i;
...
void twi_stop(void) {
  ...
  // We cannot use micros() from an ISR, so approximate the timeout with
  // cycle-counted delays
  const uint8_t us_per_loop = 8;
  uint32_t counter =
      (twi_timeout_us + us_per_loop - 1) / us_per_loop; // Round up
